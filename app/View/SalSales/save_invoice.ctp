...
//				if ($movementsExistance != array()){
//					$displayCreateMovements = 'none';
//					$displayGMovement = 'inline';
//					
//				}else{
//					$displayCreateMovements = 'inline';
//					$displayApproved = 'none';
//					$displayGMovement = 'none';
//				}
			?>
			<?php
			if(($documentState == 'SINVOICE_PENDANT' OR $documentState == '') AND ($reserved === '' OR $reserved === true)){
		...
		<a href="#" id="btnGenerateMovements" class="btn btn-info" style="display:inline"> Crear Movimientos</a>
		<!--	<a href="#" id="btnGenerateMovements" class="btn btn-success" style="display:<?php echo $displayCreateMovements;?>"> Crear Movimientos</a> -->
			<a href="#" id="btnApproveState" class="btn btn-success" style="display:<?php echo $displayApproved;?>"> Aprobar Nota de Remisión</a>
			...
			<a href="#" id="btnCancellState" class="btn btn-danger" style="display:<?php echo $displayCancelled;?>"> Cancelar Nota de Remisión</a>
			<?php
				$displayPrint = 'none';
				if($id <> ''){
					$displayPrint = 'inline';
	...
	<div class="widget-box">
		...
		<div class="widget-content tab-content">
			<div id="tab1" class="tab-pane active">
				...
				<?php if(($documentState == 'SINVOICE_PENDANT' OR $documentState == '')  AND ($reserved === '' OR $reserved === true)/*AND $movementsExistance == array()*/){ ?>
				...
				<?php } ?>
						...
						<table class="table table-bordered table-hover data-table" id="tablaItems">
							<thead>
								...
								<tr>
									<?php if ($delivered  === true) { ?>
										<th>Entregado / Disponible</th>
									<?php }else{?>
										<th>Entregado</th>
									<?php }?>
									<th>Backorder</th>
...
//									$stockReserved = ($salDetails[$i]['stock']) - ($salDetails[$i]['stockReserved']);
									echo '<tr id="itemRow'.$salDetails[$i]['itemId'].'w'.$salDetails[$i]['warehouseId'].'">';	//REVISAR SI NECESITA O NO WAREHOUSEId																						//type="hidden" txtWarehouseId
										...
										if(($documentState == 'SINVOICE_PENDANT' OR $documentState == '') AND ($reserved === '' OR $reserved === true)   /* AND $movementsExistance == array()*/){
											echo '<td class="columnItemsButtons">';
											if (($delivered  === false) OR ($salDetails[$i]['backorder'] > 0 AND $delivered  === true)) {
							...
							<tfoot>
								<tr>
									...
									<td></td>
									<?php if($discount > 0){ ?>
										<td colspan="2"><h6 id="totalLabel">Monto sin </br> descuento:</h6></td>
									<?php }else{ ?>
										<td colspan="2"><h4 id="totalLabel">Total:</h4></td>
									<?php }?>
								...
								<?php if($discount > 0){ 
									...
									<tr id="discountTr" >
										<td></td>
										<td></td>
										<td colspan="2"><h4>Descuento:</h4></td>
										<td><h4 id="discAmnt" ><?php echo number_format($discountedAmount, 2, '.', '').' Bs.'; ?></h4></td>
										<td></td>
										<td></td>
										<td></td>
										<?php if(($documentState == 'NOTE_PENDANT' OR $documentState == '') AND ($reserved === '' OR $reserved === true)){ ?>
											<td class="columnItemsButtons"></td>
										<?php }?>
									</tr>
									<tr id="totalTr" >
